...
class TestEntrypoints(unittest.TestCase):
    def test_output_pdb(self):
        mv = ModifierParameters.model_validate({"input_pdb": Path('text.pdb'),
                                               "modification": ['C', 'O', "CB", "OD1", "ND2"],
                                               "output_pdb": Path('text.pdb')})
        self.assertEqual(mv.input_pdb.suffix, ".pdb")
        ...
        with self.assertRaises(ValidationError):
            ModifierParameters.model_validate({"input_pdb": 2,
                                               "modification": ['C', 'O', "CB", "OD1", "ND2"],
                                               "output_pdb": Path('text.pdb')})

        # modification ending needs to be a list of strings, execution should fail otherwise
        with self.assertRaises(ValidationError):
            ModifierParameters.model_validate({"input_pdb": Path('text.pdb'),
